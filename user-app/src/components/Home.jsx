    ...
    <div className="space-y-10 animate-fade-in">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="w-full md:w-64 flex-shrink-0 sticky top-28 z-30 bg-[#F8FAFC]/90 backdrop-blur-md pt-2 pb-4 md:py-0 md:bg-transparent">
          ...
          <div className="flex-1 w-full">
            <h2 className="text-3xl font-black text-slate-900 mb-8 tracking-tight">{activeCategory}</h2>
            
            {displayedProducts.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-[2rem] border border-dashed border-slate-200">
                <p className="text-slate-400 font-bold text-lg">No products found in this category.</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
                {displayedProducts.map(product => {
                  ...
                  return (
                    <div key={product.id} className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden group">
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          ...
                          {qty === 0 ? (
                            <button 
                              ...
                              className="w-10 h-10 bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white rounded-full flex items-center justify-center font-black transition-all shadow-sm hover:shadow-emerald-500/30"
                            >
                              +
                            </button>
                          ) : (
